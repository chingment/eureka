    ...
    public class HttpUtil
    {
        ...
        public string HttpPost(string urlString, string body = "", string safeAccount = "", string safeAccountPwd = "")
        {
            ...
            catch (Exception ex)
            {
                if (logger != null)
                {
                    logger.CallApi(String.Format("错误，url {0}，原因 {1}", urlString, ex.Message), ex);
                }
            }
            finally
            {
                ...
                if (logger != null)
                {
                    logger.CallApi(strLog.ToString());
                }
            }
            return result;
        }

        public string HttpPostJson(string urlString, string body = "", string safeAccount = "", string safeAccountPwd = "")
        {
            string result = "";
            StringBuilder strLog = new StringBuilder();
            strLog.Append("\r\n");
        ...
        /// <returns></returns>
        public string HttpGet(string urlString, Encoding encoding, string safeAccount, string safeAccountPwd)
        {
            //定义局部变量
            ...
            try
            {
                ...
                if (!string.IsNullOrWhiteSpace(safeAccount))
                {
                    ...
                    httpWebRequest.Headers.Add("Authorization", "Basic " + Convert.ToBase64String(new ASCIIEncoding().GetBytes(usernamePassword)));
                }
