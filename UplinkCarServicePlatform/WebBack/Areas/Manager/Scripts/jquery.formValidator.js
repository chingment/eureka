...
(function ($) {
                ...
                if (html.length = 0) {
                    ...
                    if (typeof prevObj != 'undefined') {
                        ...
                        if (tagName == 'TEXTAREA') {
                            ...
                            if (showclass == "onEmpty" || showclass == "onNone") {
                                tip.css('line-height', '0px');
                            }
                            else {
                                tip.css('line-height', '24px');
                            }
                        }
                    }
                    ...
                    if (showclass != "") {
                        tip.html(showmsg).removeClass().addClass(showclass).show();
                    } else {
                        tip.html(showmsg).show();
                    }
                }
                var stype = elem.type;
        ...
        showMessage: function (returnObj) {
            ...
            if (!isValid) {
                ...
                if (empty) {
                    ...
                    if (returnObj.valLen > 0) {
                        $.formValidator.setTipState(elem, "onCorrect", showmsg);
                    }
                    else {

                        $.formValidator.setTipState(elem, "onNone", showmsg);
                    }
                }
                else {
                    $.formValidator.setTipState(elem, "onCorrect", showmsg);
                        ...
                        if (returnObj) {
                            ...
                            if (initConfig.mode != "AlertTip") {
                                ...
                                if (error_tip.indexOf("^" + tipID + "^") == -1) {
                                    ...
                                    // alert(returnObj.setting.empty)
                                    if (returnObj.setting.empty == true) {
                                        $.formValidator.setTipState(this, "onEmpty", "&nbsp;");
                                    }
                                    else {
        ...
        regexValid: function (returnObj) {
            ...
            if (elem.settings[0].empty && $.trim(elem.value) == "") {
                ...
                $.each(regexArray, function () {
                    ...
                    if (!setting.empty.isUseLeftRightEmptySize) {
                        v = v.replace(/(^\s*)|(\s*$)/g, "");
                    }

                    isValid = (new RegExp(r, setting.param)).test(v);
                ...
                case "radio":
                    ...
                    if (type == "size") {		//获得输入的字符长度，并进行校验
                        ...
                        if (sType == "text") {
                            if (!setting.empty.isUseLeftRightEmptySize) {
                                val = oldVal;
                                val = val.replace(/(^\s*)|(\s*$)/g, "").length;
                            }

                            if (!setting.empty.isUseMiddleEmptySize) {
                                val = oldVal;
                                val = oldVal.replace(/\s+/g, "").length;
                            }
                        }
                    }
    ...
    $.fn.defaultPassed = function (onShow) {
        return this.each(function () {
            ...
            var isEmpty = false;
            if (this.settings[0].empty != 'undefined') {
                isEmpty = this.settings[0].empty;
            }

            if (isEmpty) {


                var val = trimLeft($(this).val());
                if (val.length != 0) {
                    $.formValidator.setTipState(this, "onCorrect", "&nbsp;");
                }
                else {
                    $.formValidator.setTipState(this, "onNone", "");
                }
            }
            else {
    ...
    //设置必须填或不填
    $.fn.setInputRequired = function (isRquired) {
        return this.each(function () {
            if (this.settings) {

                if (!isRquired) {
                    if (typeof this.settings[1] != "undefined") {
                        if (typeof this.settings[1].min != "undefined") {
                            if (this.settings[1].min != 0) {
                                this.settings[1].oldMin = this.settings[1].min;//保存久的最小值
                                this.settings[1].min = 0;
                                this.settings[1].changeRquiredMin = true;
                            }
                        }
                    }
                }
                    ...
                    if (typeof this.settings[1] != "undefined") {
                        if (typeof this.settings[1].changeRquiredMin != "undefined") {
                            if (this.settings[1].changeRquiredMin) {
                                this.settings[1].min = this.settings[1].oldMin;//复原久的最小值
                            }
                        }
                    }
                }
...
var inputValidator_setting =
{
    ...
    empty: { leftEmpty: true, rightEmpty: true, leftEmptyError: null, rightEmptyError: null, isUseLeftRightEmptySize: false, isUseMiddleEmptySize: true }
};
...
var regexValidator_setting =
{
    ...
    empty: { isUseLeftRightEmptySize: false }
};
